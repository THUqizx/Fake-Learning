...
### Load the token collocation spike value matrix ###
from .modify_logits import DispersionLogitsProcessor, TokenCollocation
token_collocation = TokenCollocation

if token_collocation:
    print("Loading the token collocation spike value matrix...")
    from scipy.sparse import coo_matrix
    from scipy.io import mmread, mmwrite, mminfo    
    _TOKEN_COLLOCATION_MATRIX_PATH = "./TokenCollocationSpikeValues_Matrix.mtx"
    token_collocation_matrix = mmread(_TOKEN_COLLOCATION_MATRIX_PATH)
    token_collocation_matrix = token_collocation_matrix.toarray()
    print("The token collocation spike value matrix loaded.")
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        ### Add DispersionLogitsProcessor to LogitsProcessorList ###
        if token_collocation:
            dispersion_logits_processor = DispersionLogitsProcessor(
                token_collocation_matrix = token_collocation_matrix
            )
            if logits_processor is None:
                logits_processor = LogitsProcessorList([dispersion_logits_processor])
            else:
                logits_processor.append(dispersion_logits_processor)
        ### END ###
